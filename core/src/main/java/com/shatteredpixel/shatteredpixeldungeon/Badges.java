...
public class Badges {
	...
	public static void validateItemLevelAquired( Item item ) {
		...
		// 2) When an item is upgraded (scroll, weapon, or wand)
	...
	public static void validateCatalogBadges(){
		...
		if (totalThings > 0 && totalSeen == totalThings) {
			unlock(badge);
	...
	public static void validateDuelistUnlock(){
		if (!isUnlocked(Badge.UNLOCK_DUELIST) && Dungeon.hero != null
				...
				&& ((MeleeWeapon) Dungeon.hero.belongings.weapon).tier >= 1
	...
	public static String showCompletionProgress( Badge badge ){
		...
		if (badge == Badge.BOSS_SLAIN_1_ALL_CLASSES){
			for (HeroClass cls : HeroClass.all()){
				result += "\n";
			...
			for (HeroSubClass cls : HeroSubClass.all()){
