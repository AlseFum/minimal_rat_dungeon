...
public class Bones {
	...
	public static void leave() {
		...
		bundle.put( HERO_CLASS, heroClass.id );
		...
		if (depth == -1) {
			...
			try {
				...
				if (depth > 0) {
					...
					if (bundle.contains(HERO_CLASS)){
						heroClass = HeroClass.find(bundle.getString(HERO_CLASS)) != null ? HeroClass.find(bundle.getString(HERO_CLASS)) : HeroClass.WARRIOR;
					} else {
						heroClass = null;
					}
				}
			...
			if (lootAtCurLevel()) {
				...
				if (item instanceof Artifact){
					if (Loot.removeArtifact(((Artifact)item).getClass())) {
