...
public class Dungeon {
	...
	public static Level newLevel() {
		...
		DungeonRegion region = DungeonRegion.resolve();
		Level level = region.createLevel();

		//this assumes that depth stays in the range 0 to 999
		...
		//level.create() 已内置于 region.createLevel()（变体在生成前 roll 并挂载）
		region.configureLevel(level);
	...
	public static long seedForDepth(int depth, int branch){
		int lookAhead = depth;
		lookAhead += 6*branch; //six main nodes, followed by the one retained branch
	...
	public static boolean shopOnLevel() {
		return currentRegion().shopOnLevel();
	}

	public static boolean bossLevel() {
		return currentRegion().isBossLevel();
	}
	...
	private static DungeonRegion currentRegion() {
		if (level != null && level.regionId != null) {
			DungeonRegion region = DungeonRegion.find(level.regionId);
			if (region != null) return region;
		}
		return DungeonRegion.resolve();
	}

	//value used for scaling of damage values and other effects.
	//is usually the dungeon depth, but uses the final showcase node when ascending
	...
	public static boolean hasteNeeded() {
		// Keep two Haste potions per five-floor run so the retained buff is easy to experience.
		int hasteLeftThisSet = 2 - (LimitedDrops.HASTE_POTIONS.count - (depth / 5) * 2);
		if (hasteLeftThisSet <= 0) return false;

		int floorThisSet = (depth % 5);

		//Haste drops every two floors, (numbers 1-2, and 3-4) with a 50% chance for the earlier one each time.
		int targetHasteLeft = 2 - floorThisSet/2;
		if (floorThisSet % 2 == 1 && Random.Int(2) == 0) targetHasteLeft --;

		if (targetHasteLeft < hasteLeftThisSet) return true;
		else return false;
