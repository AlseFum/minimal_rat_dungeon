...
public abstract class Char extends Actor {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if (enemy.isInvulnerable(getClass())) {
			...
			//统一伤害结算：攻击管线已算好的伤害作为 solidDamage 进入修改链，
			//归因（offender/weapon/way）一并传入，供全局伤害钩子使用
			Proc.damage(new DamageInfo()
					.offender(this)
					.defender(enemy)
					.weapon(this instanceof Hero ? ((Hero) this).belongings.attackingWeapon() : null)
					.way(Dungeon.level.adjacent(pos, enemy.pos) ? DamageWay.MELEE : DamageWay.RANGED)
					.type(DamageType.PHYSICAL)
					.solidDamage(effectiveDamage)
					.reducedInAttack(true));
	...
	public void damage( int dmg, Object src ) {
		Proc.damage(Proc.fromLegacy(this, dmg, src));
	}
	...
	public float resist( Class effect ){
		...
		return result * Ring.resist(this, effect);
	}
	...
	public enum Property{
		...
		ICY ( new HashSet<Class>( Arrays.asList(WandOfFrost.class)),
