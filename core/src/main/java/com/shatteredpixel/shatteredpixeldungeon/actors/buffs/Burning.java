	...
	public boolean act() {
		...
		if (acted && Dungeon.level.water[target.pos] && !target.flying){
			...
			if (target instanceof Hero
				...
				Proc.damage(new DamageInfo().defender(hero).way(DamageWay.DOT).type(DamageType.FIRE).solidDamage(damage));
				burnIncrement++;
				...
				Proc.damage(new DamageInfo().defender(target).way(DamageWay.DOT).type(DamageType.FIRE).solidDamage(damage));
			}
