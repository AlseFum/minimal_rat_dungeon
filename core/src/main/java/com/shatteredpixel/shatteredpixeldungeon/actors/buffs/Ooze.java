	...
	public boolean act() {
		...
		if (acted && Dungeon.level.water[target.pos] && !target.flying){
			...
			if (Dungeon.scalingDepth() > 5) {
				Proc.damage(new DamageInfo().defender(target).way(DamageWay.DOT).type(DamageType.ACID).solidDamage(1 + Dungeon.scalingDepth() / 5));
			} else if (Dungeon.scalingDepth() == 5){
				Proc.damage(new DamageInfo().defender(target).way(DamageWay.DOT).type(DamageType.ACID).solidDamage(1)); //1 dmg per turn vs Goo
			} else if (Random.Int(2) == 0) {
				Proc.damage(new DamageInfo().defender(target).way(DamageWay.DOT).type(DamageType.ACID).solidDamage(1)); //0.5 dmg per turn in sewers
			}
