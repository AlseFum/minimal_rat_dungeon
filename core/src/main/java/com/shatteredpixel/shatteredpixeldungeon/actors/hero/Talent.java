...
public enum Talent {
	...
	public int icon(){
		if (this == HEROIC_ENERGY){
			...
			return cls == null ? 26 : cls.heroicEnergyIcon;
		} else {
			return icon;
		}
	}
	...
	private static final String TALENT_LAYERS = "talent_layers";
	private static final String TALENT_LAYER = "talent_layer_";
	private static final String REPLACEMENTS = "replacements";

	public static void storeTalentsInBundle( Bundle bundle, Hero hero ){

		//层 id 数组（保插入顺序）
		bundle.put(TALENT_LAYERS, hero.talentLayers.keySet().toArray(new String[0]));

		for (Hero.TalentLayerState state : hero.talentLayers.values()){
			Bundle layerBundle = new Bundle();
			for (Talent talent : state.points.keySet()){
				if (state.points.get(talent) > 0){
					layerBundle.put(talent.name(), state.points.get(talent));
				}
			}
			bundle.put(TALENT_LAYER + state.layer.id, layerBundle);
		}
	...
	public static void restoreTalentsFromBundle( Bundle bundle, Hero hero ){
		...
		//恢复层（先恢复 replacements，addTalentLayer 时会应用替换）
		String[] layerIds = bundle.contains(TALENT_LAYERS)
				? bundle.getStringArray(TALENT_LAYERS) : null;

		if (layerIds == null){
			//旧格式防御路径（版本门槛作废后正常不会走到）
			if (hero.heroClass != null){
				for (String id : hero.heroClass.talentLayerIds) hero.addTalentLayer(id);
			}
			if (hero.armorAbility != null){
				hero.addTalentLayer(hero.armorAbility.talentLayerId());
			}
		} else {
			for (String id : layerIds){
				hero.addTalentLayer(id); //未知 id 内部静默忽略
			}
		}

		//子职业天赋并入职业 t3 层（同池；读档后重做合并）
		if (hero.subClass != null){
			hero.mergeSubclassTalents(hero.subClass);
		}

		//叠加已存点数
		for (String id : hero.talentLayers.keySet()){
			Bundle layerBundle = bundle.contains(TALENT_LAYER + id)
					? bundle.getBundle(TALENT_LAYER + id) : null;
			if (layerBundle == null) continue;

			Hero.TalentLayerState state = hero.talentLayers.get(id);
			for (String tName : layerBundle.getKeys()){
				int points = layerBundle.getInt(tName);
				...
				if (!removedTalents.contains(tName)){
					try {
						...
						if (state.points.containsKey(talent)){
							state.points.put(talent, Math.min(points, talent.maxPoints()));
						}
					} catch (Exception e) {
						ShatteredPixelDungeon.reportException(e);
					}
