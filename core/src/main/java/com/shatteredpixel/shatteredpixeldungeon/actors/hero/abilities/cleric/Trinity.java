...
public class Trinity extends ArmorAbility {
	...
	public class WndUseTrinity extends WndTitledMessage {
		...
		public WndUseTrinity(Armor armor) {
			...
			if (bodyForm != null){
				...
				if (bodyForm instanceof Weapon.Enchantment){
					...
					if (Dungeon.hero.belongings.weapon() != null) {
						...
						btnBody.icon(new ItemSprite(ItemSpriteSheet.SWORD, ((Weapon.Enchantment) bodyForm).glowing()));
					}
			...
			if (spiritForm != null){
				RedButton btnSpirit = new RedButton(Messages.get(WndUseTrinity.class, "spirit",
					...
					@Override
					protected void onClick() {
						if (isDuplicateSpiritEffect(spiritForm)){
							GLog.w(Messages.get(Trinity.class, "no_duplicate"));
							hide();
							return;
						}
	...
	private static boolean isDuplicateSpiritEffect(Bundlable selected) {
		if (selected instanceof Ring) {
			Ring ring = (Ring) selected;
			return (Dungeon.hero.belongings.ring() != null
					&& ring.sameIdentity(Dungeon.hero.belongings.ring()))
					|| (Dungeon.hero.belongings.misc() instanceof Ring
					&& ring.sameIdentity((Ring) Dungeon.hero.belongings.misc()));
		}
		Class<?> selectedClass = selected.getClass();
		return (Dungeon.hero.belongings.ring() != null
				&& Dungeon.hero.belongings.ring().getClass() == selectedClass)
				|| (Dungeon.hero.belongings.misc() != null
				&& Dungeon.hero.belongings.misc().getClass() == selectedClass)
				|| (Dungeon.hero.belongings.artifact() != null
				&& Dungeon.hero.belongings.artifact().getClass() == selectedClass);
	}
	...
	public static class WndItemtypeSelect extends WndTitledMessage {
		...
		public WndItemtypeSelect(HolyTome tome, ClericSpell spell) {
			...
			for (Class<?> cls : Statistics.itemTypesDiscovered) {
				if (spell == BodyForm.INSTANCE
						&& (Weapon.Enchantment.class.isAssignableFrom(cls)
						|| Armor.Glyph.class.isAssignableFrom(cls))) {
					discoveredClasses.add(cls);
				} else if (spell == MindForm.INSTANCE
						&& (Wand.class.isAssignableFrom(cls)
						|| MissileWeapon.class.isAssignableFrom(cls))) {
					discoveredClasses.add(cls);
				} else if (spell == SpiritForm.INSTANCE
						&& cls != HolyTome.class
						&& Artifact.class.isAssignableFrom(cls)) {
					discoveredClasses.add(cls);
				}
			}
			...
			if (spell == SpiritForm.INSTANCE) {
				options.addAll(Ring.getKnown());
			}
