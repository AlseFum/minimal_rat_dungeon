	...
	private void perCharEffect(ConeAOE cone, Hero hero, Char primaryTarget, Weapon.Enchantment ench) {
		...
		if (ench == null) {
			for (Char ch : affected){
				Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(powerMulti* Hero.heroDamageIntRange(6, 12))));
			}
		...
		//*** Kinetic ***
			...
			if (storedKineticDamage > 0) {
				for (Char ch : affected) {
					if (ch != primaryTarget) {
						Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(storedKineticDamage * 0.4f * powerMulti)));
					}
				}
		...
		//*** Projecting ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(hero.damageRoll() * 0.3f * powerMulti)));
				}
			}
		...
		//*** Grim ***
			...
			for (Char ch : affected){
				if (ch != primaryTarget) {
					...
					if (Random.Float() < chance*powerMulti){
						Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.GRIM).type(DamageType.PHYSICAL).trueDamage(ch.HP));
		...
		//*** Polarized ***
			...
			for (Char ch : affected){
				if (Random.Float() < 0.5f*powerMulti){
					Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Hero.heroDamageIntRange(24, 36)));
				}
			}
