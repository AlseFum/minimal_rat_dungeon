						...
						if (tracker.depth == Dungeon.depth && tracker.branch == Dungeon.branch){
							...
							if (existing != null && existing != hero){
								if (hero.hasTalent(Talent.TELEFRAG)){
									...
									Proc.damage(new DamageInfo().offender(hero).defender(hero).way(DamageWay.SELF).type(DamageType.PHYSICAL).solidDamage(Math.min(heroDmg, heroHP-1)));
									...
									Proc.damage(new DamageInfo().offender(hero).defender(existing).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(damage));
