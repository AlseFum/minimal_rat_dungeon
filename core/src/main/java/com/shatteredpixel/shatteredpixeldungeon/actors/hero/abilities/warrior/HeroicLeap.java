...
public class HeroicLeap extends ArmorAbility {
	...
	@Override
	public void activate( Armor armor, Hero hero, Integer target ) {
		if (target != null) {
					...
					for (int i : PathFinder.NEIGHBOURS8) {
						...
						if (mob != null && mob != hero && mob.alignment != Char.Alignment.ALLY) {
							if (hero.hasTalent(Talent.BODY_SLAM)){
								...
								damage -= mob.drRoll();
								Proc.damage(new DamageInfo().offender(hero).defender(mob).way(DamageWay.MELEE).type(DamageType.PHYSICAL).solidDamage(damage));
							}
