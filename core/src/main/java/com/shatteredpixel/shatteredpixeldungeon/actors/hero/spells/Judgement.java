			...
			public void call() {
				...
				for (Char ch : Actor.chars()){
					if (ch.alignment != hero.alignment && Dungeon.level.heroFOV[ch.pos]){
						Proc.damage(new DamageInfo().offender(hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Hero.heroDamageIntRange(damageBase, 2*damageBase)));
