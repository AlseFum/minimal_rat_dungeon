		...
		if (ch != null) {
			...
			if (Char.hasProp(ch, Char.Property.UNDEAD) || Char.hasProp(ch, Char.Property.DEMONIC)){
				if (hero.pointsInTalent(Talent.SUNRAY) == 2) {
					Proc.damage(new DamageInfo().offender(Dungeon.hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(12));
				} else {
					Proc.damage(new DamageInfo().offender(Dungeon.hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(8));
				}
			} else {
				if (hero.pointsInTalent(Talent.SUNRAY) == 2) {
					Proc.damage(new DamageInfo().offender(Dungeon.hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Hero.heroDamageIntRange(6, 12)));
				} else {
					Proc.damage(new DamageInfo().offender(Dungeon.hero).defender(ch).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Hero.heroDamageIntRange(4, 8)));
				}
			}
