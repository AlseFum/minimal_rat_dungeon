...
public class MirrorImage extends NPC {
	...
	public int attackSkill( Char target ) {
		...
		attackSkill *= Ring.accuracyMultiplier(hero);
	...
	public int defenseSkill(Char enemy) {
		if (hero != null) {
			...
			int heroEvasion = (int)((4 + hero.lvl) * Ring.evasionMultiplier( hero ));
	...
	public int attackProc( Char enemy, int damage ) {
		...
		if (hero.belongings.weapon() != null){
			...
			if (hero.buff(BodyForm.BodyFormBuff.class) != null
				...
				damage = hero.buff(BodyForm.BodyFormBuff.class).enchant().proc(new Sword(), this, enemy, damage);
			}
			if (!wasEnemy || enemy.alignment == Alignment.ENEMY) {
				if (hero.buff(HolyWeapon.HolyWepBuff.class) != null) {
					...
					Proc.damage(new DamageInfo().offender(this).defender(enemy).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(dmg * Weapon.Enchantment.genericProcChanceMultiplier(this))));
				}
			}
			return damage;
		}
