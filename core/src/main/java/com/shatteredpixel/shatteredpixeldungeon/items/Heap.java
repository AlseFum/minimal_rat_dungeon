...
public class Heap implements Bundlable {
	...
	public void open( Hero hero ) {
		switch (type) {
		case TOMB:
			Rat.spawnAround( hero.pos, Rat.Variant.REVIVER );
			break;
		case REMAINS:
		case SKELETON:
		...
		if (haunted){
			if (Rat.spawnAt( pos, Rat.Variant.REVIVER ) == null) {
				hero.sprite.emitter().burst( ShadowParticle.CURSE, 6 );
				Proc.damage(new DamageInfo().defender(hero).way(DamageWay.ENV).type(DamageType.PHYSICAL).solidDamage(hero.HP / 2).icon(FloatingText.PHYS_DMG_NO_BLOCK));
				if (!hero.isAlive()){
				Dungeon.fail(Rat.class);
				GLog.n( Messages.capitalize(Messages.get(Char.class, "kill", Messages.get(Rat.class, "name"))));
				}
			}
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		//remove a duplicate guidebook if its only page was already read
		for (Item item : items.toArray(new Item[0])){
