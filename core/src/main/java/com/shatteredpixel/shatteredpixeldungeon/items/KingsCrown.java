...
public class KingsCrown extends Item {
	...
	public void upgradeArmor(Hero hero, Armor armor, ArmorAbility ability) {
		...
		if (armor != null){
			...
			Armor upgradedArmor = Armor.upgrade(hero, armor);
			if (hero.belongings.armor == armor) {

				hero.belongings.armor = upgradedArmor;
				((HeroSprite) hero.sprite).updateArmor();
				upgradedArmor.activate(hero);

			} else {

				armor.detach(hero.belongings.backpack);
				upgradedArmor.collect(hero.belongings.backpack);

			}
		}

		hero.armorAbility = ability;
		hero.addTalentLayer(ability.talentLayerId());
