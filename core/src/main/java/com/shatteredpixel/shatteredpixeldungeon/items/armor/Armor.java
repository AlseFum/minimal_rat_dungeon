...
public class Armor extends EquipableItem {
	...
	/** Shared ability charge retained after class-specific armor types were removed. */
	...
	{
		// A single generic armor is the remaining equipment example.
	...
	public String defaultAction() {
		if (Dungeon.hero != null && Dungeon.hero.belongings != null && isEquipped(Dungeon.hero)) {
			return Dungeon.hero.armorAbility == null ? AC_UNEQUIP : AC_ABILITY;
		}
		return AC_EQUIP;
	}

	/**
	 * Reuses an existing armor's state when the crown/ability flow upgrades it.
	 * Class-specific armor shells no longer exist in the five-level build.
	 */
	public static Armor upgrade(Hero owner, Armor source) {
		if (source.charger != null) {
			source.charger.detach();
			source.charger = null;
		}
		Armor result = new Armor(Math.max(1, source.tier));
		result.level(source.trueLevel());
		result.augment = source.augment;
		result.inscribe(source.glyph);
		result.seal = source.seal;
		result.glyphHardened = source.glyphHardened;
		result.cursed = source.cursed;
		result.curseInfusionBonus = source.curseInfusionBonus;
		result.masteryPotionBonus = source.masteryPotionBonus;
		result.charge = 50;
		if (source.levelKnown && source.cursedKnown) {
			result.identify();
		} else {
	...
	private static final String ABILITY_CHARGE      = "ability_charge";
	...
	public void storeInBundle( Bundle bundle ) {
		...
		bundle.put( ABILITY_CHARGE, charge );
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		charge = bundle.getFloat( ABILITY_CHARGE );
	...
	public ArrayList<String> actions(Hero hero) {
		...
		if (isEquipped(hero) && hero.armorAbility != null) actions.add(AC_ABILITY);
	...
	public void execute(Hero hero, String action) {
		...
		if (action.equals(AC_DETACH) && seal != null){
			...
			updateQuickslot();
		} else if (action.equals(AC_ABILITY)
				...
				if (charge >= chargeUse) {
					...
					usesTargeting = false;
					GLog.w(Messages.get(Armor.class, "low_charge"));
				}
			}
	}
	...
	public String actionName(String action, Hero hero) {
		if (AC_ABILITY.equals(action) && hero.armorAbility != null) {
	...
	public String status() {
		if (Dungeon.hero == null || Dungeon.hero.belongings == null
				|| !isEquipped(Dungeon.hero) || Dungeon.hero.armorAbility == null) {
			return super.status();
		}
	...
	public void activate(Char ch) {
		if (ch instanceof Hero) {
			// Resurrection detaches transient buffs without clearing item fields.
	...
	public int STRReq(int lvl){
		// Five-level showcase: armor no longer adds two strength per tier.
		return Hero.STARTING_STR;
	}

	@Override
	public int targetingPos(Hero user, int dst) {
		return user.armorAbility == null ? super.targetingPos(user, dst) : user.armorAbility.targetedPos(user, dst);
	}
	...
	public String desc() {
		...
		if (Dungeon.hero != null && Dungeon.hero.belongings.contains(this)
			...
			desc += "\n\n" + Dungeon.hero.armorAbility.shortDesc();
		}
		return desc;
	}

	/** Regenerates the shared ability charge while the armor is equipped. */
	public class Charger extends Buff {
		...
		public boolean act() {
			if (Regeneration.regenOn()) {
				charge += 100 / 500f * Ring.armorChargeMultiplier(target);
				if (charge > 100) charge = 100;
				Armor.this.updateQuickslot();
			}
			spend(TICK);
			return true;
		}
	...
	public static abstract class Glyph implements Bundlable {
		...
		public static float genericProcChanceMultiplier( Char defender ){
			float multi = Ring.enchantPowerMultiplier(defender);
