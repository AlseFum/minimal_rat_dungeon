...
public class DriedRose extends Artifact {
		...
		public boolean act() {
			...
			if (ghost != null && !cursed && target.buff(MagicImmune.class) == null){
				...
				if (ghost.HP < ghost.HT && Regeneration.regenOn()) {
					partialCharge += (ghost.HT / 500f) * Ring.artifactChargeMultiplier(target);
					updateQuickslot();
			...
			if (charge < chargeCap
				...
				partialCharge += (1/5f * Ring.artifactChargeMultiplier(target));
				while (partialCharge > 1){
					charge++;
					partialCharge--;
					if (charge == chargeCap){
				...
				if (spawnPoints.size() > 0) {
					Rat.spawnAt(Random.element(spawnPoints), Rat.Variant.REVIVER);
	...
	public static class GhostHero extends DirectableAlly {
		...
		public void sayAppeared(){
			if (Dungeon.hero.buff(AscensionChallenge.class) != null){
				yell( Messages.get( this, "dialogue_ascension_" + Random.IntRange(1, 6) ));
					...
					// The five-level showcase has one sewer theme; keep the Rose's
					// original dialogue cadence without referencing removed regions.
					int variant = Dungeon.depth == 1 ? Random.IntRange(1, 3) : Random.IntRange(1, 6);
		...
		public void sayBoss(){
				// Goo is the only boss in the condensed route.
