...
public class SkeletonKey extends Artifact {
	...
	public class keyRecharge extends ArtifactBuff {
		...
		public boolean act() {
			if (charge < chargeCap
				...
				chargeGain *= Ring.artifactChargeMultiplier(target);
	...
	public static class KeyReplacementTracker extends Buff {
		...
		public void processExcessKeys(){
			...
			if (keysNeeded >= 0) {
				while (Notes.keyCount(new Key(Key.Kind.IRON, Dungeon.depth)) > keysNeeded) {
					Notes.remove(new Key(Key.Kind.IRON, Dungeon.depth));
					removed = true;
				}
			}
			keysNeeded = goldenKeysNeeded[Dungeon.depth];
			if (keysNeeded >= 0) {
				while (Notes.keyCount(new Key(Key.Kind.GOLDEN, Dungeon.depth)) > keysNeeded) {
					Notes.remove(new Key(Key.Kind.GOLDEN, Dungeon.depth));
					removed = true;
				}
			}
			keysNeeded = crystalKeysNeeded[Dungeon.depth];
			if (keysNeeded >= 0) {
				while (Notes.keyCount(new Key(Key.Kind.CRYSTAL, Dungeon.depth)) > keysNeeded) {
					Notes.remove(new Key(Key.Kind.CRYSTAL, Dungeon.depth));
					removed = true;
				}
			}
			if (removed){
