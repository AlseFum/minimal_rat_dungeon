...
public class Bomb extends Item {
	...
	public void explode(int cell){
		...
		if (explodesDestructively()) {
			...
			for (Char ch : affectedChars){
				...
				if (dmg > 0) {
					Proc.damage(new DamageInfo().defender(ch).way(DamageWay.ENV).type(DamageType.PHYSICAL).solidDamage(dmg));
				}
	...
	public static class EnhanceBomb extends Recipe {
		...
		public static final LinkedHashMap<Class<? extends Item>, BombRecipe> validIngredients = new LinkedHashMap<>();
		static {
			validIngredients.put(PotionOfFrost.class,
					new BombRecipe(0, () -> new PayloadBomb(PayloadBomb.Payload.FROST)));
			validIngredients.put(ScrollOfMirrorImage.class, new BombRecipe(0, WoollyBomb::new));
			validIngredients.put(PotionOfLiquidFlame.class,
					new BombRecipe(1, () -> new PayloadBomb(PayloadBomb.Payload.FIRE)));
			validIngredients.put(ScrollOfRage.class, new BombRecipe(1, Noisemaker::new));
			validIngredients.put(PotionOfHaste.class,
					new BombRecipe(2, () -> new PayloadBomb(PayloadBomb.Payload.SMOKE)));
			validIngredients.put(ScrollOfRecharging.class,
					new BombRecipe(2, () -> new PayloadBomb(PayloadBomb.Payload.FLASH)));
			validIngredients.put(PotionOfHealing.class, new BombRecipe(3, RegrowthBomb::new));
			validIngredients.put(ScrollOfRemoveCurse.class, new BombRecipe(3, HolyBomb::new));
			validIngredients.put(GooBlob.class,
					new BombRecipe(6, () -> new PayloadBomb(PayloadBomb.Payload.ARCANE)));
			validIngredients.put(LiquidMetal.class,
					new BombRecipe(6, () -> new PayloadBomb(PayloadBomb.Payload.SHRAPNEL)));
		}
		...
		public int cost(ArrayList<Item> ingredients) {
			for (Item i : ingredients){
				if (validIngredients.containsKey(i.getClass())){
					return validIngredients.get(i.getClass()).cost;
				}
			}
			return 0;
		}
		...
		public Item brew(ArrayList<Item> ingredients) {
			...
			for (Item i : ingredients){
				...
				if (validIngredients.containsKey(i.getClass())){
					result = validIngredients.get(i.getClass()).factory.create();
					if (i instanceof GooBlob || i instanceof LiquidMetal) Catalog.countUse(i.getClass());
				}
			}
		...
		public Item sampleOutput(ArrayList<Item> ingredients) {
			for (Item i : ingredients){
				if (validIngredients.containsKey(i.getClass())){
					return validIngredients.get(i.getClass()).factory.create();
				}
			}
			return null;
		}

		public static class BombRecipe {
			private final int cost;
			private final BombFactory factory;

			private BombRecipe(int cost, BombFactory factory) {
				this.cost = cost;
				this.factory = factory;
			}
		}

		private interface BombFactory {
			Bomb create();
		}
	}
}
