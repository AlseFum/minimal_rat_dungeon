...
public class Pickaxe extends MeleeWeapon {
			...
			public void call() {
				int damageBoost = 0;
					if (Char.hasProp(enemy, Char.Property.INORGANIC)
										|| (enemy instanceof Rat && isMiningVariant((Rat) enemy))) {
	...
	private boolean isMiningVariant(Rat rat) {
		Rat.Variant variant = rat.variant();
		return variant == Rat.Variant.SPLITTER
				|| variant == Rat.Variant.SHELL
				|| variant == Rat.Variant.CORROSIVE
				|| variant == Rat.Variant.WEB_KITER;
	}
