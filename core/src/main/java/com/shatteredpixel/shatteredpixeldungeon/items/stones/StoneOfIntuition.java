...
public class StoneOfIntuition extends InventoryStone {
	...
	private static Item curGuess = null;
		...
		public WndGuess(final Item item){
			...
			final RedButton guess = new RedButton(""){
				...
				protected void onClick() {
					super.onClick();
					useAnimation();
					boolean correct = item instanceof Ring && curGuess instanceof Ring
							? ((Ring) item).sameIdentity((Ring) curGuess)
							: curGuess != null && item.getClass() == curGuess.getClass();
					if (correct){
						if (item instanceof Ring){
							((Ring) item).setKnown();
			...
			final ArrayList<Item> unIDed = new ArrayList<>();
			if (item.isIdentified()){
				hide();
				return;
			} else if (item instanceof Potion){
				for (Class<? extends Item> candidate : Potion.getUnknown()) {
					unIDed.add(Reflection.newInstance(candidate));
				}
			} else if (item instanceof Scroll){
				if (item instanceof ExoticScroll) {
					for (Class<? extends Item> candidate : Scroll.getUnknown()){
						unIDed.add(Reflection.newInstance(ExoticScroll.regToExo.get(candidate)));
					}
				} else {
					for (Class<? extends Item> candidate : Scroll.getUnknown()) {
						unIDed.add(Reflection.newInstance(candidate));
			...
			for (final Item candidate : unIDed){
				...
				IconButton btn = new IconButton(){
					...
					protected void onClick() {
						curGuess = candidate;
						guess.visible = true;
						String name = candidate instanceof Ring
								? ((Ring) candidate).identityName()
								: Messages.get(candidate, "name");
						guess.text(Messages.titleCase(name));
						guess.enable(true);
						super.onClick();
					}
				};
				Image im = new Image(Assets.Sprites.ITEM_ICONS);
				im.frame(ItemSpriteSheet.Icons.film.get(candidate.icon));
				im.scale.set(2f);
				btn.icon(im);
