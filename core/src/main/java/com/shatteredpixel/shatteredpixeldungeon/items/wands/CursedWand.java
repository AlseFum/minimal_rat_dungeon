...
public class CursedWand {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			switch (Random.Int(3)) {
				case 0: default:
					...
					return true;
				case 1:
					new ExplosiveTrap().set(bolt.collisionPos).activate();
					return true;
				case 2:
					new TeleportationTrap().set(bolt.collisionPos).activate();
					return true;
			}
		}
	}
	...
	public static class HealthTransfer extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			if (target != null) {
				...
				Proc.damage(new DamageInfo().defender(toDamage).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(damage));
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			...
			for (Char ch : affected){
				...
				if (ch.alignment != Char.Alignment.ALLY || !positiveOnly){
					...
					Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.ELECTRIC).solidDamage(Random.NormalIntRange(5 + Dungeon.scalingDepth() / 4, 10 + Dungeon.scalingDepth() / 4)));
					if (ch.isAlive()) {
	...
	public static class Levitate extends CursedEffect {
		...
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			Char ch = Actor.findChar(bolt.collisionPos);
			if (!positiveOnly && ch != null && !ch.flying && !Char.hasProp(ch, Char.Property.IMMOVABLE)) {
			...
			for (int i = 0; i < Dungeon.level.length(); i++){
				if (fieldOfView[i] && !Dungeon.level.solid[i]){
					...
					if (Actor.findChar(i) != null){
						...
						Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.FIRE).solidDamage(dmg));
					}
				...
				if (positiveOnly && ch.alignment == Char.Alignment.ALLY){
					...
					switch (Random.Int(5)){
						case 0: default:
							...
							break;
						case 1:
							Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.FROST).solidDamage(dmg));
						...
						case 2:
							...
							Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.POISON).solidDamage(dmg));
							ch.sprite.emitter().burst(PoisonParticle.SPLASH, 20);
							break;
						case 3:
							Ooze ooze = Buff.affect(ch, Ooze.class);
							ooze.set(Ooze.DURATION);
							Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.ACID).solidDamage(dmg));
							Splash.at( ch.sprite.center(), 0x000000, 20 );
							break;
						case 4:
							Proc.damage(new DamageInfo().defender(ch).way(DamageWay.MAGIC).type(DamageType.ELECTRIC).solidDamage(dmg));
	...
	static {
		...
		VERY_RARE_EFFECTS.add(new SpawnAmbushRat());
	...
	public static class SpawnAmbushRat extends CursedEffect {
		@Override
		public boolean effect(Item origin, Char user, Ballistica bolt, boolean positiveOnly) {
			Char ch = Actor.findChar(bolt.collisionPos);
			...
			Rat rat = Rat.of(Rat.Variant.AMBUSH);
			rat.pos = spawnCell;
			rat.state = rat.HUNTING;
			CellEmitter.get(rat.pos).burst(Speck.factory(Speck.STAR), 10);
			GameScene.add(rat);

			//mimic is enthralled, but also contains no extra reward, if positive only
			if (positiveOnly){
				Buff.affect(rat, ScrollOfSirensSong.Enthralled.class);
			} else {
				Item reward;
				do {
