...
public abstract class Wand extends Item {
	...
	protected static void wandProc(Char target, int wandLevel, int chargesUsed){
		...
		if (Dungeon.hero.subClass == HeroSubClass.PRIEST && target.buff(GuidingLight.Illuminated.class) != null) {
			...
			Proc.damage(new DamageInfo().offender(Dungeon.hero).defender(target).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Dungeon.hero.lvl+5));
		}
	...
	public class Charger extends Buff {
		...
		private void recharge(){
			...
			if (Regeneration.regenOn())
				partialCharge += (1f/turnsToCharge) * Ring.wandChargeMultiplier(target);
