...
public class WandOfBlastWave extends DamageWand {
	...
	public void onZap(Ballistica bolt) {
		...
		//Press all tiles in the area before applying knockback.
		...
		for (int i  : PathFinder.NEIGHBOURS8){
			...
			if (ch != null){
				...
				if (ch.alignment != Char.Alignment.ALLY) Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(damageRoll()));
		...
		if (ch != null){
			wandProc(ch, chargesPerCast());
			Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(damageRoll()));
		...
		Actor.add(new Pushing(ch, ch.pos, newPos, new Callback() {
			public void call() {
				...
				if (finalCollided && ch.isActive()) {
					Proc.damage(new DamageInfo().offender(curUser).defender(ch).way(DamageWay.ENV).type(DamageType.PHYSICAL).solidDamage(Random.NormalIntRange(finalDist, 2*finalDist)).icon(FloatingText.PHYS_DMG_NO_BLOCK));
					if (ch.isActive()) {
	...
	public void onHit(MagesStaff staff, Char attacker, Char defender, int damage) {
		...
		if (defender.buff(Paralysis.class) != null && defender.buff(BWaveOnHitTracker.class) == null){
			...
			Proc.damage(new DamageInfo().offender(attacker).defender(defender).weapon(this).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(procChanceMultiplier(attacker) * dmg)));
