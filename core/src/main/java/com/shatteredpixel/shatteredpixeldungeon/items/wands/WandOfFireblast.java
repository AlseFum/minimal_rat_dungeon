...
public class WandOfFireblast extends DamageWand {
	...
	public void onZap(Ballistica bolt) {
		...
		for ( Char ch : affectedChars ){
			...
			Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.FIRE).solidDamage(damageRoll()));
			if (ch.isAlive()) {
				...
				for (int i : PathFinder.NEIGHBOURS9) {
					...
					if (ch != null) {
						...
						if (ch.alignment == Char.Alignment.ENEMY) {
							...
							Proc.damage(new DamageInfo().offender(attacker).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.FIRE).solidDamage(Math.round(powerMulti*Hero.heroDamageIntRange(2 + 2*buffedLvl(), 8 + 4*buffedLvl()))));
						}
					}
				}
			}
