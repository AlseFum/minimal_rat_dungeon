	...
	public void onZap(Ballistica bolt) {
		...
		for (Char ch : affected){
			...
			if (ch == curUser && ch.isAlive()) {
				Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.ELECTRIC).solidDamage(Math.round(damageRoll() * multiplier * 0.5f)));
				if (!curUser.isAlive()) {
				...
				Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.ELECTRIC).solidDamage(Math.round(damageRoll() * multiplier)));
			}
		}
	}
