...
public class WandOfPrismaticLight extends DamageWand {
	...
	private void affectTarget(Char ch){
		...
		if (ch.properties().contains(Char.Property.DEMONIC) || ch.properties().contains(Char.Property.UNDEAD)){
			...
			Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(Math.round(dmg*1.333f)));
		} else {
			ch.sprite.centerEmitter().burst( RainbowParticle.BURST, 10+buffedLvl() );

			Proc.damage(new DamageInfo().offender(curUser).defender(ch).weapon(this).way(DamageWay.MAGIC).type(DamageType.PHYSICAL).solidDamage(dmg));
		}
