...
abstract public class Weapon extends KindOfWeapon {
		...
		if (attacker.buff(MagicImmune.class) == null) {
			...
			if (attacker instanceof Hero && isEquipped((Hero) attacker)
				...
				if (defender.isAlive() && !becameAlly) {
					...
					Proc.damage(new DamageInfo()
							.offender(attacker).defender(defender).weapon(this)
							.way(DamageWay.MAGIC).type(DamageType.PHYSICAL)
							.solidDamage(Math.round(dmg * Enchantment.genericProcChanceMultiplier(attacker))));
				}
			...
			if (attacker instanceof Hero && isEquipped((Hero) attacker) &&
				...
				Proc.damage(new DamageInfo()
						.offender(attacker).defender(defender).weapon(this)
						.way(DamageWay.MAGIC).type(DamageType.PHYSICAL)
						.solidDamage(Smite.bonusDmg((Hero) attacker, defender)));
			}
		}
	...
	protected float speedMultiplier(Char owner ){
		float multi = Ring.attackSpeedMultiplier(owner);
	...
	protected static int STRReq(int tier, int lvl){
		// Five-level showcase: special weapons share the same fixed requirement.
	...
	protected String name(String baseName) {
		if (isEquipped(Dungeon.hero) && !hasCurseEnchant() && Dungeon.hero.buff(HolyWeapon.HolyWepBuff.class) != null
			&& (Dungeon.hero.subClass != HeroSubClass.PALADIN || enchantment == null)){
				...
				return enchantment != null && (cursedKnown || !enchantment.curse()) ? enchantment.name(baseName) : baseName;
	...
	public static abstract class Enchantment implements Bundlable {
		...
		public static float genericProcChanceMultiplier( Char attacker ){
			float multi = Ring.enchantPowerMultiplier(attacker);
