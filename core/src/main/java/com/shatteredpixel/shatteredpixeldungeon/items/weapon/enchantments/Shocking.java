		...
		if (Random.Float() < procChance) {
			...
			for (Char ch : affected) {
				if (ch.alignment != attacker.alignment) {
					Proc.damage(new DamageInfo().offender(attacker).defender(ch).weapon(weapon).way(DamageWay.MAGIC).type(DamageType.ELECTRIC).solidDamage(Math.round(damage * 0.5f * powerMulti)));
				}
			}
