...
abstract public class MissileWeapon extends Weapon {
	...
	public int min() {
		if (Dungeon.hero != null){
			return Math.max(0, min(buffedLvl() + Ring.levelDamageBonus(Dungeon.hero)));
		} else {
	...
	public int max() {
		if (Dungeon.hero != null){
			return Math.max(0, max( buffedLvl() + Ring.levelDamageBonus(Dungeon.hero) ));
		} else {
	...
	public int STRReq(int lvl){
		// Five-level showcase: equipment has one fixed strength requirement.
	...
	public float durabilityPerUse( int level ){
		...
		if (Dungeon.hero != null) usages *= Ring.durabilityMultiplier( Dungeon.hero );
