...
public class MineLargeRoom extends CaveRoom {
	...
	public void paint(Level level) {
		...
		if (Blacksmith.Quest.Type() == Blacksmith.Quest.CRYSTAL){
			...
			Rat m = Rat.of(Rat.Variant.CRYSTAL);
			...
			Rat s = Rat.of(Rat.Variant.GEOMANCER);
			s.pos = sapperPos;
			level.mobs.add(s);
			...
			Rat g = Rat.of(Rat.Variant.GEOMANCER);
			g.pos = guardPos;
			level.mobs.add(g);
			...
			for (int i = 0; i < traps; i ++){
				...
				level.setTrap(new MiningRockfallTrap().reveal(), level.pointToCell(r));
			}
			...
			Point p = center();
			Rat m = Rat.of(Rat.Variant.STENCH);
			m.pos = level.pointToCell(p);
			level.mobs.add(m);
