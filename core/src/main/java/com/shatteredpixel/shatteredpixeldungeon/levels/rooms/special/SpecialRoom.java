...
public abstract class SpecialRoom extends Room {
	...
	//One representative per distinct room mechanic. Laboratory rooms are injected separately.
	private static final ArrayList<Class<? extends SpecialRoom>> ALL_SPECIALS = new ArrayList<>( Arrays.asList(
			WeakFloorRoom.class, PoolRoom.class, SentryRoom.class, StatueRoom.class,
			CrystalChoiceRoom.class, SacrificeRoom.class, GardenRoom.class, MagicWellRoom.class,
			ToxicGasRoom.class, MagicalFireRoom.class, TrapsRoom.class
	));
	...
	public static final ArrayList<Class<? extends SpecialRoom>> CRYSTAL_KEY_SPECIALS = new ArrayList<>( Arrays.asList(
			PitRoom.class, CrystalChoiceRoom.class
	) );
	...
	private static final ArrayList<Class<? extends SpecialRoom>> POTION_SPAWN_ROOMS = new ArrayList<>( Arrays.asList(
			PoolRoom.class, SentryRoom.class, ToxicGasRoom.class, MagicalFireRoom.class, TrapsRoom.class
	) );
	...
	public static void initForRun() {
		runSpecials = new ArrayList<>(ALL_SPECIALS);
	...
	public static void restoreRoomsFromBundle( Bundle bundle ) {
		...
		if (bundle.contains( ROOMS )) {
			for (Class<? extends Room> type : bundle.getClassArray(ROOMS)) {
				if (type != null && ALL_SPECIALS.contains(type)) {
					runSpecials.add(type);
				}
			}
			for (Class<? extends SpecialRoom> type : ALL_SPECIALS) {
				if (!runSpecials.contains(type)) {
					runSpecials.add(type);
				}
			}
		} else {
