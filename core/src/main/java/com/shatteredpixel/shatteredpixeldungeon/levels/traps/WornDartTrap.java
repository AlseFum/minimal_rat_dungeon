			...
			protected boolean act() {
				...
				if (target != null) {
					...
					if (Dungeon.level.heroFOV[pos] || Dungeon.level.heroFOV[target.pos]) {
						((MissileSprite) ShatteredPixelDungeon.scene().recycle(MissileSprite.class)).
								reset(pos, finalTarget.sprite, new Dart(), new Callback() {
									...
									public void call() {
										...
										Proc.damage(new DamageInfo().defender(finalTarget).way(DamageWay.TRAP).type(DamageType.PHYSICAL).solidDamage(dmg));
						...
						Proc.damage(new DamageInfo().defender(finalTarget).way(DamageWay.TRAP).type(DamageType.PHYSICAL).solidDamage(Random.NormalIntRange(4, 8) - finalTarget.drRoll()));
						return true;
					}
				} else {
					return true;
