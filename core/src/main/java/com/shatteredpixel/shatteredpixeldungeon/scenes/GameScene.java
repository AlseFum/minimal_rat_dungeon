...
public class GameScene extends PixelScene {
	...
	private final HashMap<Entity, ItemSprite> entitySprites = new HashMap<>();
	private Group heaps;
	private Group mobs;
	...
	public void create() {
		...
		SPDSettings.lastClass(java.util.Arrays.asList(HeroClass.all()).indexOf(Dungeon.hero.heroClass));
		...
		entities = new Group();
		add(entities);
		for (Entity entity : Dungeon.level.entities.valueList()) {
		...
		if (InterlevelScene.mode != InterlevelScene.Mode.NONE) {
			...
			if (Dungeon.hero.hasTalent(Talent.ROGUES_FORESIGHT)
				...
				int reqSecrets = Dungeon.level.hasVariant(RegionVariant.SECRETS) ? 2 : 1;
			...
			for (Hero.TalentLayerState state : Dungeon.hero.talentLayers.values()){
				if (Dungeon.hero.talentPointsAvailable(state.layer) > 0){
					unspentTalents = true;
					break;
				}
			}
			...
			//逐变体播报并记录地标（可多个组合）
			for (RegionVariant variant : Dungeon.level.variants) {
				GLog.w(variant.desc());
				if (variant == RegionVariant.CHASM)        Notes.add(Notes.Landmark.CHASM_FLOOR);
				else if (variant == RegionVariant.WATER)   Notes.add(Notes.Landmark.WATER_FLOOR);
				else if (variant == RegionVariant.GRASS)   Notes.add(Notes.Landmark.GRASS_FLOOR);
				else if (variant == RegionVariant.DARK)    Notes.add(Notes.Landmark.DARK_FLOOR);
				else if (variant == RegionVariant.LARGE)   Notes.add(Notes.Landmark.LARGE_FLOOR);
				else if (variant == RegionVariant.TRAPS)   Notes.add(Notes.Landmark.TRAPS_FLOOR);
				else if (variant == RegionVariant.SECRETS) Notes.add(Notes.Landmark.SECRETS_FLOOR);
			}
	...
	private void addEntitySprite( Entity entity ) {
		if (entity == null || entities == null || entitySprites.containsKey(entity)) return;
		ItemSprite sprite = new ItemSprite(entity.image());
		sprite.place(entity.pos);
		sprite.visible = Dungeon.level.heroFOV[entity.pos];
		entities.add(sprite);
		entitySprites.put(entity, sprite);
	}

	private void removeEntitySprite( Entity entity ) {
		ItemSprite sprite = entitySprites.remove(entity);
		if (sprite != null) sprite.killAndErase();
	}

	private void updateEntitySprites() {
		for (Entity entity : Dungeon.level.entities.valueList()) {
			ItemSprite sprite = entitySprites.get(entity);
			if (sprite != null) sprite.visible = Dungeon.level.heroFOV[entity.pos];
		}
	}
	...
	public static void add( Entity entity ) {
		if (scene != null) scene.addEntitySprite(entity);
	}

	public static void remove( Entity entity ) {
		if (scene != null) scene.removeEntitySprite(entity);
	}
	...
	public static void flashForDocument( Document doc, String page ){
		if (scene != null) {
			if (doc == Document.ADVENTURERS_GUIDE && page.equals(Document.GUIDE_INTRO)){
