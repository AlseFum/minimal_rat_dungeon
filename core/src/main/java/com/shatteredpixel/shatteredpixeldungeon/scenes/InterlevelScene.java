...
public class InterlevelScene extends PixelScene {
	...
	public void create() {
		...
		switch (mode){
			...
			//The showcase has one visual region; keep a separate marker for the non-combat
			//ending node so its final black fade still works without old region assets.
			int region = loadingDepth == 6 ? 6 : 1;
		...
		//for portrait users, each run the sewer splash changes what details it focuses on
	...
	private void descend() throws IOException {
		if (curTransition == null && Dungeon.hero != null) {
	...
	private void fall() throws IOException {
		if (Dungeon.branch != 0 || Dungeon.depth >= 5) {
			//There is no ordinary floor below the retained branch or the Goo floor.
			//Return to the current level entrance if a future room reintroduces a pit;
			//never leave the hero on an unresolved falling transition.
