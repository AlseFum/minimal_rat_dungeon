...
public enum Icons {
	...
	public static Image get( HeroClass cl ) {
		return cl == null ? null : cl.iconImage();
	}

	public static Image get(RegionVariant variant){
		if (variant == RegionVariant.CHASM)   return get(DEPTH_CHASM);
		if (variant == RegionVariant.WATER)   return get(DEPTH_WATER);
		if (variant == RegionVariant.GRASS)   return get(DEPTH_GRASS);
		if (variant == RegionVariant.DARK)    return get(DEPTH_DARK);
		if (variant == RegionVariant.LARGE)   return get(DEPTH_LARGE);
		if (variant == RegionVariant.TRAPS)   return get(DEPTH_TRAPS);
		if (variant == RegionVariant.SECRETS) return get(DEPTH_SECRETS);
		return get(DEPTH);
	}

	public static Image getLarge(RegionVariant variant){
		if (variant == RegionVariant.CHASM)   return get(STAIRS_CHASM);
		if (variant == RegionVariant.WATER)   return get(STAIRS_WATER);
		if (variant == RegionVariant.GRASS)   return get(STAIRS_GRASS);
		if (variant == RegionVariant.DARK)    return get(STAIRS_DARK);
		if (variant == RegionVariant.LARGE)   return get(STAIRS_LARGE);
		if (variant == RegionVariant.TRAPS)   return get(STAIRS_TRAPS);
		if (variant == RegionVariant.SECRETS) return get(STAIRS_SECRETS);
		return get(STAIRS);
	}
}
