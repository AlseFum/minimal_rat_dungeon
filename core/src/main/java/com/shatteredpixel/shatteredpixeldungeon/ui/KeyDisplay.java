...
public class KeyDisplay extends Visual {
	...
	public void updateKeys(){
		// Slot 0 is the black missed-key marker; Kind owns the four stable sprite slots.
		keys = new int[Key.Kind.values().length + 1];
		...
		for (Notes.KeyRecord rec : Notes.getRecords(Notes.KeyRecord.class)){
			if (rec.depth() < Dungeon.depth){
				...
				keys[rec.kind().displaySlot()] += rec.quantity();
			}
		}
	...
	private void updateVertices(){
		...
		while (totalKeys > maxKeys){
			int mostSlot = 1;
			int mostNum = 0;
			for (Key.Kind kind : Key.Kind.values()){
				int slot = kind.displaySlot();
				if (keys[slot] >= mostNum){
					mostSlot = slot;
					mostNum = keys[slot];
				}
			}
