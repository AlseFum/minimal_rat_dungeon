...
public class MenuPane extends Component {
	...
	protected void createChildren() {
		...
		//多变体：取第一个变体作深度按钮的展示（无变体 = 普通层）
		RegionVariant displayVariant = Dungeon.level.variants.length > 0 ? Dungeon.level.variants[0] : null;

		depthIcon = Icons.get(displayVariant);
		add(depthIcon);
		...
		depthButton = new Button(){
			...
			protected void onClick() {
				...
				if (displayVariant == null){
					...
					GameScene.show(new WndTitledMessage(Icons.getLarge(displayVariant),
							Messages.titleCase(displayVariant.title()),
							displayVariant.desc()));
				}
			}
		};
	...
	private static class JournalButton extends Button {
		...
		protected void onClick() {
			...
			if (flashingPage != null){
				if (flashingDoc.pageNames().contains(flashingPage)){
