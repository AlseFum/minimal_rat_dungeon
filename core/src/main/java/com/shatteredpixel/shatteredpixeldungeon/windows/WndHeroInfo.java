...
public class WndHeroInfo extends WndTabbed {
	...
	private static class HeroInfoTab extends Component {
		...
		public HeroInfoTab(HeroClass cls){
			...
			String[] desc_entries = new String[]{cls.shortDesc()};
	...
	private static class TalentInfoTab extends Component {
		...
		public TalentInfoTab( HeroClass cls ){
			...
			//职业层预览（零点数）；按徽章进度过滤已解锁层
			ArrayList<Hero.TalentLayerState> states = new ArrayList<>();
			for (String id : cls.talentLayerIds){
				TalentLayer layer = TalentLayer.find(id);
				if (layer != null){
					states.add(new Hero.TalentLayerState(layer));
				}
			}

			talentPane = new TalentsPane(TalentButton.Mode.INFO, states, TalentInfoTab::badgeGate);
			add(talentPane);
		}

		/** 预览徽章门槛：t2 需 LEVEL_REACHED_2+BOSS_SLAIN_2，t3 需 BOSS_SLAIN_4 */
		private static boolean badgeGate(TalentLayer layer){
			int unlock = layer.levelThresholds()[0];
			if (unlock <= 2) return true;
