...
public class WndRanking extends WndTabbed {
	...
	private class StatsTab extends Group {
		...
		public StatsTab() {
			...
			String heroClass = record.heroClass.id;
			if (Dungeon.hero != null){
	...
	private class TalentsTab extends Group{
		...
		public TalentsTab(){
			...
			//INFO 模式按层过滤：只显示当前等级应解锁的层（不再破坏性裁剪 hero 数据）
			TalentsPane p = new TalentsPane(TalentButton.Mode.INFO, Dungeon.hero.talentLayers.values(), layer -> {
				int unlock = layer.levelThresholds()[0];
				if (unlock <= 2) return true;
				if (Dungeon.hero.lvl >= 6 && unlock <= 7) return true;
				if (Dungeon.hero.lvl >= 12 && unlock <= 13) return true;
				return Dungeon.hero.lvl >= 20 && unlock <= 21;
			});
			add(p);
			p.setPos(0, 0);
